...
function commit(transaction?:Uint8Array|string, writes?:api.IWrite[], err?:Error) : TransactionStep {
...
function rollback(transaction?:Uint8Array|string, err?:Error)  : TransactionStep {
  const proto = {
    database: DATABASE_ROOT,
    transaction: transactionId(transaction),
  };
...
function begin(transaction?:Uint8Array|string, prevTransaction?:Uint8Array|string, err?:Error)  : TransactionStep{
...
function runTransaction<T>(
    transactionCallback: (transaction:Transaction, docRef:DocumentReference) => Promise<T>,
    ...expectedRequests) {
  const overrides = {
    beginTransaction: (actual:api.IBeginTransactionRequest, _, callback:(err:Error, resp:api.IBeginTransactionResponse) => void) => {
    ...
    commit: (actual:api.ICommitRequest, _, callback:(err:Error, resp:api.ICommitResponse) => void) => {
      const request = expectedRequests.shift();
      expect(request.type).to.equal('commit');
      expect(actual).to.deep.eq(request.request);
      callback(request.error, request.response);
    },
    rollback: (actual:api.IRollbackRequest, _, callback:(err:Error) => void) => {
      const request = expectedRequests.shift();
...
describe('failed transactions', () => {
  ...
  it('requires a promise', () => {
    return expect(
               runTransaction(
                   (() => {}) as InvalidApiUsage, begin(), rollback(new Buffer('foo'))))
  ...
  it('fails on beginTransaction', () => {
    return expect(
               runTransaction(
                   ...
                   begin(
                       'foo', undefined, new Error('Fails (1) on beginTransaction')),
                   begin(
                       'foo', undefined, new Error('Fails (2) on beginTransaction')),
                   begin(
                       'foo', undefined, new Error('Fails (3) on beginTransaction')),
                   begin(
                       'foo', undefined, new Error('Fails (4) on beginTransaction')),
                   begin(
                       'foo', undefined,
